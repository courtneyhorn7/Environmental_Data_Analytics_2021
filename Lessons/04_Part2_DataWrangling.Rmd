...
library(tidyverse)
library(lubridate)
  #below, we import the processed data set
NTL.phys.data.PeterPaul <- read.csv("./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv")
#this data set has all the original info but only for peter and paul lake
...
# Data summaries for physical data
  #want to check the parameters of the data you imported, even if you just processed it
...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake")
  #that means paul lake or peter lake
...
#str gives the modes of the columns
...
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y")
  #year is two digits here, so lower case y
...
#might want to rearrange the table using these

#gather is transforming columns into rows and making the data longer
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration")
  #making two columns in the above code
...
count(NTL.nutrient.data.PeterPaul.gathered, nutrient)
  #subset does something similar to filtering
  #is.na checks whether there are NAs and returns a logical operator
  #R counted by nutrient in the above code
...
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, sampledate, c("Y", "m", "d"))
  #sample date is the column we want to seperate here
...
#can use join function to join data even if not all column names match

```{r}

NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset)
  #R gives output with info on how it joined them
  #full join combines all rows and columns from x and y
...
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
  #the path is to the folder where all the files you want to read are located
#the pattern indicates which files you want to read
#fullnames = true indicates you want to use the full names of the files

LitterFiles

Litter <- LitterFiles %>%
  ldply(read.csv)
#ldply is part of plyr package
```
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID"))
  #were joining by siteID and things
  #litter trap has same number of obs as litter but more variables
...
#can split data sets according to groupings
```{r}
#the data set used below is the combined one
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#by using the &, your asking for rows where none of those things are na
