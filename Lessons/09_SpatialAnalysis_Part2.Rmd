...
#we grouped by sites so that we could make it where each location only has one point

View(epa_pm25_sites_sf)
...
st_bbox(counties_sf)
#this creates a spatial bounding box around all the features
...
#coloring our features on the relative amount of land
```
...
#North American Datum 1983
#EPSG 4269
 #the two data frames do not use the same CRS
...
huc8_sf1 <- st_read("./Data/Spatial/NCHUC8.shp")

huc8_sf1 <- filter(huc8_sf, SUBBASIN == "Upper Neuse")
unique(huc8_sf$SUBBASIN)
...
mapView(huc8_sf, col.regions = 'orange') +
  mapView(huc8_sf1, col.regions = 'purple')
#i had to fidget with the layers to get the purple one to go on top
...
#this code counts the different types
...
#FIPS is a character or a variable (both work equally well)
#the FIPS = factor code tells R to read it as a factor. strings as factors wouldn't work here because R would just think its a number
...
#GEOID and FIPS are the two columns being used to join the data sets
...
enviro_atlas <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv")
colnames(enviro_atlas)
enviro_atlas <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c('HUC_12' = 'factor')) %>% 
...
HUC_8_joined <- left_join(huc8_sf, enviro_atlas)

#or?
huc_sf_wind <- huc8_sf %>% 
  left_join(enviro_atlas, by = c('HUC_8', 'HUC_8'))
View(HUC_8_joined)
View(huc_sf_wind)


#View the outputs
mapView(huc_sf_wind, zcol = "kWhkm2day")
#the one above isn't doing the map correctly for some reason, but the one below is
mapView(HUC_8_joined, zcol = "kWhkm2day")
...
mapview(huc8_sf)
#so huc8_sf is the HUC_8 data were talking about
colnames(huc8_sf)
...
#this figures out which counties intersect this neuse shape
