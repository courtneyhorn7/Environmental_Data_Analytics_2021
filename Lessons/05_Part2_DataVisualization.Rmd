...
print(O3plot) #this also has the default ggplot theme
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #grey lines/axes on white background
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #classic looking theme, with x and y axis and no gridline
print(O3plot4)
```
...
If you would like to set a common theme across all plots in your analysis session, you may define a theme and call up that theme for each graph. This eliminates the need to add multiple lines of code in each plot. 
  #the theme set up allows you to change multiple aspects of the plot
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top") #alternative: legend.position + legend.justification
#the default legend location for ggplot is right
#legend justification affects right, center , or left alignment
...
print(O3plot5)  
#now text is all black and larger

theme_set(mytheme) #set theme for all subsequent plots
#then you can run all plots and the theme will happen, even when you don't specify the theme within the plot
...
print(O3plot7)  
#hlines are horizontal, and this one crosses at 50. lty specifies the type of the line
#this here is a scatter plot, and the points rep the data site
#made point size 1.5
#x is being printed on jan 2020, and y printed at 45
#text will be bold, added the word good
#why are we doing as.Date here?
#want month to be abbreviated format (%b), want year to be two digits



 #*what should you specify within the aes?
...
#try to choose palletes to communicate what you want your audience to percieve
...
NvsP <-
  ...
  geom_point() 
print(NvsP)
#here, we are plotting two nutrients and categorizing them by depth and changing shape of points
...
NvsP2 <-
  ...
  ylab("ugh") + xlab("UGH")# change your legend labels here
...
#we changed shape of points, and since we have two variables, we have to choose two shapes (15 and 17)
#we used magma option for depth
#legend now on right
#legend is now biggr (size 12)
#we changed the shapes
Nutrientplot <-
...
library(cowplot)
#can use cowplot package to put multiple plots on one page
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1))
#plot grid allows you to divide your plot window into as many cells as you want
#rel_heights adjusts the space given to each plot. Can size your plots
...
#specify file name with relative path, width, heigh, and somethin else, resolution
