...
#stringasfactors = true makes it where summary will work for character entries
...
head(NTL.phys.data$sampledate)
  #the output of date is year - month -date order
```
...
```{r, results = "hide"}
vignette("dplyr")
#this code gave me the 'Intro to dplyr' page in the help window
  #Rows
#filter()
#slice()
#arrange()
...
#rename()
#mutate()
  #mutate allows you to create a new column based on the values of other columns
...
`==`
#use the above when trying to compare a value to a specific value
`!=`use this to check whether variables are diff

`<`check whether less than
`<=`
`>`
`>=`
`&`Use this if you need to check multiple conditions and want them both to be true
`|`use this if you need to check multiple conditions and one or the other should be true

```{r}
class(NTL.phys.data$lakeid)
  #gonna test class v factor a bit - NTL replica is how im testing it
NTLreplica <- NTL.phys.data
NTLreplica$lakeid <- as.character(NTLreplica$lakeid)
class(NTL.phys.data$depth)
class(NTLreplica$lakeid)
  #whats the difference between a factor and a character?


# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]
  #0 is before the comma bc we filtering by rows
...
# Did the methods arrive at the same result?
    #all of the above methods got to the same result
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
  #here, we are selecting all instances of Paul Lake OR Peter Lake
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
  ...
  #here, we are selecting for rows which do not have the above lake name values
...
#we choose 151, because 152 is the first day of June, and we want it to be included
...
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304))
  #%in% is like a value matching operator
# Exercise: 
...
#phys.data is the column we want to arrange by
...
#since the three columns we are getting are consequtive columns (sample date, depth, temp), we can do this (sampledate:temperature_C)
...
#the lubridate month function can extract the month from a date value
...
#select can be used to pull columns from a data set or to change the order of the columns

# find out the start and end dates of the dataset
#the interval function creates an interval object something with specific start and end dates
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613])
#the first row will be the first observed date, and the last row will be the last observed date
interval(sampledate:temperature_C, last(NTL.phys.data.PeterPaul1$sampledate))
```
