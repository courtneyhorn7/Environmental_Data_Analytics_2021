...
--------------|---------------------
              |
ECOTOX_Neonicotinoids | Chemical information
EPAair_03_NC | Air pollution information
NIWO Litter | blah
...
Lake_nutrients_data$sampledate <-as.Date(Lake_nutrients_data$sampledate, "")
?as.Date
...
summary(Lake_nutrients_data_subset)
...
Lake_nutrients_data_subset_summary_Phos <- 
    Lake_nutrients_data_subset %>% 
    group_by(lakename) %>% 
    summarise(total_phos = sum(tp_ug), 
              minimum_phos = min(tp_ug),
              max_phos = max(tp_ug),
              sd_phos = sd(tp_ug))
View(Lake_nutrients_data_subset_summary_N)
View(Lake_nutrients_data_subset_summary_Phos)
```
...
```{r}
?kable
kable(Lake_nutrients_data_subset_summary_N, caption = "Nitrogen Summary Table")
kable(Lake_nutrients_data_subset_summary_Phos, caption = "Phosphorus Summary Table")
```
...
```{r}
ggplot(data=Lake_nutrients_data_subset_summary_N, aes(x = ))
