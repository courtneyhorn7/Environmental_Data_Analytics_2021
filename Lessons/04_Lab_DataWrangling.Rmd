...
NTL.phys.data.Tues<-filter(lakename %>%in%>% c("Tuesday Lake")) %>%, (NTL.phys.data,year4 %>% in %>% c(1990:1999))

NTL.phys.data.Tues<-filter(NTL.phys.data, year4 %in% c("1990:1999"))
summary(NTL.phys.data.1999)

NTL.phys.data_Tues1 <- filter(NTL.phys.data, lakename == "Tuesday Lake")
NTL.phys.data_Tues2 <- filter(NTL.phys.data, year4 == c("1990:1999"))
summary(NTL.phys.data_Tues2)
...
NTL.phys.data.processed <- 
  ...
  #NTL.phys.data is the initial data set

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 through 1999 only for July 
NTLData4 <- NTL.phys.data %>% filter(lakename %in% c("Tuesday Lake")) %>% filter(year4 %in% c(1990:1999)) %>% filter (month %in% c("July"))

mutate(temperature_F = (temperature_C*9/5) + 32)

SurTemp <- NTL.phys.data %>% filter(depth == 0) %>% drop_na(temperature_C) %>% summarise(Mean_Surf_T = mean(temperature_C))
View(SurTemp)
...
attempt <- NTL.phys.data %>% mutate(temperature_F = (temperature_C*9/5) + 32)
attempt

NTLDAta <- NTL.phys.data %>%  filter(lakename %in% c("Tuesday Lake")) %>% filter(year4 %in% c(1990:1999)) %>% mutate(Month = month(sampledate)) %>%  filter (Month %in% c(7))
View(NTLDAta)
...
NTLirradiance <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration")

NTLirradiance <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "samplinglocation", values_to = "irradiance")
View(NTLirradiance)
