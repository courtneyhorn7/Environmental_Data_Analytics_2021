...
colnames(PeterPaul.chem.nutrients)
...
E1 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate,y=temperature_C,shape=as.factor(year4)))+
  geom_point()+
  facet_wrap(vars(lakename), nrow=2)+
  ylab(expression("surface_temperature_c"))
print(E1)
...
ggplot(PeterPaul.chem.nutrients, aes(y=temperature_c, x=sampledate, color=depth))+
  geom_point(size = 0.5) +
  scale_color_viridis(option = "magma", direction = -1) + xlab("Date")


ggplot(PeterPaul.chem.nutrients, aes(y=temperature_c, x=sampledate, color=depth)) + scale_color_viridis(option = "magma", direction = -1)
```
...
# Add a line of best fit for the linear regression of these variables.
  ...
  ##my attempt that didn't work great but worked a bit
plot3 <-
  ggplot(EPAair, aes(x = PM2.5, y = Ozone)) +
  geom_point(aes(color = SITE_LATITUDE, alpha = 0.1))
print(plot3)
