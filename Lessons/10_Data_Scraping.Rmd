...
#data scraping is scraping data off websites that don't lend themselves to easily getting the data
#in these examples, the data are there, but there are no download methods really
## Set up
```{r, message = FALSE}
getwd()
library(tidyverse)
library(lubridate)
library(ggplot2)
#because well be working with tables and dates
...
#check terms of service to know if scraping is ethical
...
#i'm using the little chunk below to try to figure out why were usig the !! bang bang
...
df_withdrawals1$Registrant
df_withdrawals$Registrant
#Plot 
...
df_withdrawals <- data.frame("Month" = rep(1:12),
  ...
  mutate(Registrant = !!the_registrant,
         ...
         Facility_type = !!the_facility_type,
         Date = my(paste(Month,"-",Year)))
#!! tells R to interpret the_registrant as a variable rather than a string
...
#OR, use purrr's map function. This is an example of how to use lapply for two inputs and have one input be static
...
#the two lists have to be the same size here
...
the_df <- cross2(the_years,the_facilities) %>% 
  map(lift(scrape.it)) %>% 
  bind_rows()
#this scrapes all 9 combinations
...
ggplot(the_df,aes(x=Date,y=Avg_Withdrawals_mgd,color=Facility_name)) + 
  ...
  labs(title = "Water usage data",
       y="Withdrawal (mgd)",
       x="Date")
#plotted multiple years 3 sites
```
